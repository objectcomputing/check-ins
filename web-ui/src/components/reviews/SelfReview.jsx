...
import {
  ...
  findSelfReviewRequestsByPeriodAndTeamMember
...
const SelfReview = ({ periodId, onBack }) => {
      ...
      if (!memberProfile?.supervisorid) {
        dispatch({
          ...
          payload: {
            severity: 'error',
            toast:
              'You do not have an assigned supervisor. Please contact your HR administrator.'
          }
        });
      } else {
  ...
  return (
    ...
    selfReview.id && (
      <FeedbackSubmitForm
        requesteeName={memberProfile?.firstName + ' ' + memberProfile?.lastName}
        requestId={selfReview?.id}
        request={selfReview}
        reviewOnly={'SUBMITTED' === selfReview?.status?.toUpperCase()}
      />
    )
  );
};
