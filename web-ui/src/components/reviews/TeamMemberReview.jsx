...
const TeamMemberReview = ({selfReview, reviews, memberProfile, reloadReviews}) => {
  ...
  const handleOpenReassign = useCallback(() => setReassignOpen(true), [setReassignOpen]);
  const handleCloseReassign = useCallback(() => setReassignOpen(false), [setReassignOpen]);
  const handleOpenCancel = useCallback(() => setCancelOpen(true), [setCancelOpen]);
  const handleCloseCancel = useCallback(() => setCancelOpen(false), [setCancelOpen]);
  ...
  const handleReassign = useCallback((assignee) => {
    ...
    <Box sx={{ bgcolor: 'background.paper', width: "100%" }}>
      <AppBar position="static">
        <Tabs
          ...
          variant="fullWidth"
        >
          <Tab icon={selfReviewIcon} label={memberProfile?.firstName ? memberProfile?.firstName + "'s Self-Review" : "Self-Review"} {...a11yProps(0)} />
          {
            reviews && reviews.map((review, index) => {
              ...
              return (<Tab icon={icon} label={label} {...a11yProps(index+1)} />);
            })
          }
        </Tabs>
      </AppBar>
          ...
          reviews && reviews.map((review, index) => {
            ...
            return (
              <TabPanel value={value} index={index+1} dir={theme.direction}>
                ...
                <Button onClick={handleOpenCancel} className={classes.actionButtons} variant="outlined" color="secondary">
                  Cancel
                </Button>
                <Button onClick={handleOpenReassign} className={classes.actionButtons} variant="outlined" color="primary">
                  Reassign
                </Button>
                </div>)}
      ...
      <SelectUserModal userLabel="Reviewer" open={reassignOpen} onSelect={handleReassign} onClose={handleCloseReassign} />
      <Modal open={cancelOpen} onClose={handleCloseCancel}>
        <Card className="cancel-feedback-request-modal">
          <CardHeader title={<Typography variant="h5" fontWeight="bold">Cancel Review</Typography>}/>
          <CardContent>
            <Typography variant="body1">
              Are you sure you want to cancel the review sent to <b>{recipient?.name}</b> on <b>{review?.sendDate}</b>?
              The recipient will not be able to respond to this request once it is canceled.
            </Typography>
          </CardContent>
          <CardActions>
            <Button color="secondary" onClick={handleCloseReassign}>No, Keep Feedback Request</Button>
            <Button color="primary" onClick={handleCancelClick}>Yes, Cancel Feedback Request</Button>
          </CardActions>
        </Card>
      </Modal>
    </Box>
