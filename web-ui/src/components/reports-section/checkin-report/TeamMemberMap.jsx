...
const TeamMemberMap = ({ members, closed, planned, reportDate }) => {
  ...
  const checkinPath = (member, checkin) =>
    `/checkins/${member.id}/${checkin.id}`;
  ...
  members.map(member => {
    member.checkins = selectCheckinsForMember(state, member.id)
                ...
                >
                  <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                    {member.checkin == null ? (
                      ...
                      >
                        <Typography component="nobr" variant="h6">
                          {getCheckinDate(member.checkin).toLocaleDateString(
                            navigator.language,
                            {
                              year: 'numeric',
                              month: '2-digit',
                          ...
                          {getCheckinDate(member.checkin).getTime() >
                          new Date().getTime() ? (
                    ...
                    {member.checkin == null && member.checkins.length > 0 && (
                      <span>
                        Last Activity:{' '}
                        {
                          ...
                          >
                            {getCheckinDate(member.checkins[0])
                              .toString()
                              .split(' ')
                              .slice(0, 5)
                              .join(' ')}
