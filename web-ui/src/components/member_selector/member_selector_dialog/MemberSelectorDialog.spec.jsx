...
const initialState = {
  ...
  memberProfiles: [currentUserProfile, memberProfile, managerProfile],
  skills: [
...
const server = setupServer(
  http.get(
    `http://localhost:8080/services/guilds/members?guildId=${testGuild.id}`,
    () =>
      HttpResponse.json([
        { memberId: managerProfile.id, guildId: testGuild.id }
      ])
  )
);
...
describe('MemberSelectorDialog', () => {
  ...
  it('Filters the list of members by guild', async () => {
    ...
    optionToSelect = await screen.findByRole('option', {
      name: testGuild.name
    });
    await userEvent.click(optionToSelect);
  ...
  it('Filters the list of members by manager (with indirect reports)', async () => {
    ...
    await userEvent.click(filterTypeField);
    let optionToSelect = await screen.findByRole('option', {
      name: /manager/i
    });
    await userEvent.click(optionToSelect);
