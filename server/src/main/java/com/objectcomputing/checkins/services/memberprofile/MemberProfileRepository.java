...
public interface MemberProfileRepository extends CrudRepository<MemberProfile, UUID> {
    ...
    @Query(value = "SELECT id, " +
            ...
            "supervisorid, terminationDate, birthDate, voluntary, excluded, lastSeen " +
    ...
    "id, firstname, middlename, lastname, suffix, title, pdlid, location, workemail, employeeid, startdate, biotext, supervisorid, terminationdate, birthdate, voluntary, excluded, lastSeen 0 as level " +
    "FROM member_profile " +
    "WHERE id = :id and terminationdate is NULL " +
    "   UNION ALL " +
    "SELECT " +
    "e.id, e.firstname, e.middlename, e.lastname, e.suffix, e.title, e.pdlid, e.location, e.workemail, e.employeeid, e.startdate, e.biotext, e.supervisorid, e.terminationdate, e.birthdate, e.voluntary, e.excluded, e.lastSeen, level + 1 " +
    ...
    "s.supervisorid, s.terminationdate, s.birthdate, s.voluntary, s.excluded, s.lastSeen " +
    "s.level " +
    "FROM subordinate s " +
    "WHERE s.id <> :id " +
