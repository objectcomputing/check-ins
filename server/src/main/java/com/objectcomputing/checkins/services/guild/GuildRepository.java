...
public interface GuildRepository extends CrudRepository<Guild, UUID> {
    ...
    @Query(value = "SELECT t_.id, PGP_SYM_DECRYPT(cast(t_.name as bytea),'${aes.key}') as name, PGP_SYM_DECRYPT(cast(description as bytea),'${aes.key}') as description, PGP_SYM_DECRYPT(cast(link as bytea),'${aes.key}') as link, t_.is_community as is_community " +
            "FROM guild t_ " +
